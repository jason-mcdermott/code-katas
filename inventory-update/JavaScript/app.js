...
// arr1). If an item cannot be found, add the new item and quantity into the inventory 
...
function updateInventory(arr1, arr2) {
    ...
    arr1.forEach(
        item => map.set(item[1], { 
            item: item[1], count: item[0]
        })
    );
    
    arr2.forEach(newItem => {
        let existingItem = map.get(newItem[1]);
        if(existingItem){
            map.set(newItem[1], { 
                item: newItem[1], count: existingItem.count + newItem[0]
            })
        }
        else {
            map.set(newItem[1], { 
                item: newItem[1], count: newItem[0]
            })
        }
    })
    ...
    sorted.forEach(record => {
        results.push([record.count, record.item])
    });
